...
Class Actions extends DBConnection{
    ...
    function save_queue(){
        $code = sprintf("%'.02d",1);
        while(true){
            $chk = $this->query("SELECT count(queue_id) `count` FROM `queue_list` where queue = '".$code."' ")->fetchArray()['count'];
            if($chk > 0){
        ...
        $_POST['queue'] = $code;
        extract($_POST);
        $sql = "INSERT INTO `queue_list` (`queue`,`customer_name`) VALUES('{$queue}','client')";
        $save = $this->query($sql);
        if($save){
            $resp['status'] = 'success';
    ...
    function reset_queue(){
        $qry = $this->query("DELETE FROM queue_list");
        $resp['status']='success';
        return json_encode($resp);
    }
    ...
    function next_queue(){
        extract($_POST);
        $get = $this->query("SELECT queue_id,`queue`,customer_name FROM `queue_list` where status = 0 order by queue_id asc  limit 1");
        @$res = $get->fetchArray();
        $resp['status']='success';
        if($res){
